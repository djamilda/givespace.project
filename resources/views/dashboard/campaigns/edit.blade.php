...
<head>
    ...
    <title>Edit Kampanye - CampaignManager</title>
...
<main class="max-w-4xl mx-auto p-6 pt-24 pb-12">
    <!-- Header Section -->
    <div class="mb-8">
        <h1 class="font-headline-lg text-headline-lg text-on-surface mb-1">Edit Kampanye</h1>
        <p class="font-body-base text-body-base text-secondary">Perbarui informasi kampanye penggalangan dana Anda.</p>
    </div>
    <!-- Navigation Buttons -->
            ...
            <div class="space-y-6">
                ...
                <div class="max-w-xs">
                    ...
                    <div class="relative">
                        ...
                        <input class="w-full border @error('target_amount') border-error focus:ring-2 focus:ring-error focus:border-error @else border-outline-variant focus:ring-2 focus:ring-primary focus:border-primary @enderror rounded-md pl-10 pr-3 py-2 outline-none transition-all text-body-base" id="target_amount" name="target_amount" type="number" value="{{ old('target_amount', $campaign->target_amount) }}"/>
                    </div>
                    @error('target_amount')
                        <span class="text-error font-error-text mt-1 block">{{ $message }}</span>
                    @enderror
                ...
                <div class="flex items-center gap-3 bg-surface-container-low p-4 rounded-md border border-outline-variant">
                    <div class="flex items-center h-5">
                        <input class="h-5 w-5 text-primary border-outline-variant rounded focus:ring-primary" id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', $campaign->is_active) ? 'checked' : (session()->has('_old_input') ? '' : ($campaign->is_active ? 'checked' : '')) }} />
                    </div>
                    <div class="text-sm">
